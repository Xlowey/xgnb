...
const flags={};for(let i=0;i<30;i++){const o=w.MuseumChapterProgress.objective({flags});if(!o.room)break;const obj=r[o.room].objects.find(x=>x.id===o.entrance);assert(obj,JSON.stringify(o));console.log('route',o.room,o.entrance);const m=/scene-(\d+)/.exec(obj.scene||'');if(i===0)flags.scene03Seen=true;else if(i===1)flags.scene04Seen=true;else if(obj.type==='guard')flags.scene06Seen=true;else if(obj.type==='contract')flags.scene13Seen=true;else if(m){flags['scene'+m[1]+'Seen']=true;if(m[1]==='16')flags.scene18Seen=true;}else throw Error('unhandled '+JSON.stringify(o));}
...
assert.equal(w.MuseumStory.scenes['scene-03'].nextScene,null);assert.equal(w.MuseumStory.scenes['note-repeat'].events[0].type,'document');assert.equal(r.museum.objects.find(o=>o.id==='overview-dorm').target,'corridor');assert.equal(r.dorm.objects.filter(o=>o.scene==='scene-12').length,0);assert(!w.MuseumChapterMaps.visible(r.dorm.objects.find(o=>o.scene==='scene-11'),{flags:{}}));assert(!w.MuseumChapterMaps.visible(r.dorm.objects.find(o=>o.scene==='scene-11'),{flags:{scene09Seen:true,scene11Seen:true}}));console.log('PASS routes, spawn/entry collision, approaches, scene references, hidden future/completed events');
...
assert(!r.dorm.objects.some(o=>o.id==='dorm-scene-14'),'scene-12 has one entrance: the canteen');
...
const uniformEvents=w.MuseumStory.scenes['scene-23'].events;const uniformTrigger=uniformEvents.findIndex(e=>e.type==='dialogue'&&/纸片的意思/.test(e.text||''));const uniformItem=uniformEvents.findIndex(e=>e.item==='银色的发卡.webp');assert(uniformTrigger>=0&&uniformItem>uniformTrigger,'scene-23 discoveries must follow the line that finds them');
