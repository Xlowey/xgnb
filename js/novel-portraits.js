(function () {
  ...
  var portraits={};
  var assetVersion="48";
  [["hero","portraits/hero-portrait.png","主角立绘"],["zhaoling","portraits/zhaoling-portrait.png","赵灵立绘"],["director","portraits/director-portrait.png","馆长立绘"]].forEach(function(entry){var img=document.createElement("img");img.src=base+entry[1]+"?v="+assetVersion;img.alt=entry[2];img.draggable=false;img.className="novel-portrait portrait-"+entry[0];img.hidden=true;layer.appendChild(img);portraits[entry[0]]=img;});
  function character(speaker){if(/^主角|^玩家|^你(?:（|$)/.test(speaker))return "hero";if(/^馆长/.test(speaker))return "director";if(/^赵灵/.test(speaker))return "zhaoling";return null;}
  function render(scene,line){
    ...
    var showDirector=active==="director" || (!monologue && cast.has("director"));
    portraits.director.hidden=!showDirector;portraits.director.classList.toggle("is-speaking",active==="director");
    layer.dataset.speaker=active || "other";layer.classList.toggle("has-prop",!!line.visual);layer.classList.toggle("is-pair",showZhao || showDirector);
  }
