...
function open(options,mode){
 ...
 var cp=checkpoint();if(cp){var cpItem=document.createElement('article');cpItem.className='story-save-checkpoint';var cpInfo=document.createElement('div');var cpMeta=cp.checkpoint||{};cpInfo.textContent='选择前检查点 · '+(cpMeta.label||'重要选择')+' · '+(cpMeta.summary||'');var cpButton=button(mode==='load'?'读取':'仅供读取',function(){if(mode!=='load')return;dialog.close();options.load(cp);});cpButton.disabled=mode!=='load';cpItem.append(cpInfo,cpButton);grid.append(cpItem);}
 var all=entries();for(var i=0;i<20;i++)card('存档位 '+(i+1),all[i],i);
 }
 dialog.oncancel=function(event){event.preventDefault();close();};render();if(!dialog.open)dialog.showModal();
}
